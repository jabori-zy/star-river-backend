...
impl StarRiverErrorTrait for KeyError {
    ...
    fn error_code(&self) -> ErrorCode {
        ...
        let code = match self {
            KeyError::CoreError { .. } => 1000,                // Core error
            KeyError::InvalidKeyType { .. } => 1001,           // Invalid cache key type
            KeyError::InvalidIndicatorType { .. } => 1002,     // Invalid indicator type
            KeyError::InvalidKeyFormat { .. } => 1003,         // Invalid cache key format
            KeyError::ParseExchangeFailed { .. } => 1004,      // Parse exchange failed
            KeyError::ParseKlineIntervalFailed { .. } => 1005, // Parse kline interval failed
            KeyError::TimeRangeNotSet { .. } => 1006,          // Time range not set
        };
