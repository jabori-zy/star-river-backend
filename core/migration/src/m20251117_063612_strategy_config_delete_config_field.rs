...
impl MigrationTrait for Migration {
    async fn up(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        // SQLite does not support dropping columns directly, need to rebuild table

        // 1. Create new temporary table without config field
        manager
            .create_table(
                Table::create()
        ...
        // 2. Migrate data from original table to new table, excluding config field
        let sql = r#"
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        // Rollback: re-add config field

        // 1. Create new temporary table with config field
        manager
            .create_table(
                Table::create()
        ...
        // 2. Migrate data from current table to temporary table, set config field to NULL
        let sql = r#"
