        ...
        match language {
            ...
            Language::Chinese => {
                match self {
                    ...
                    StartNodeError::ValueNotGreaterThanOrEqualToZero { node_name, node_id, config_name, config_value, .. } => {
                        format!("开始节点 [{}({})] 配置 [{}] 应该大于等于零(>= 0)，但值为 [{}]", node_name, node_id, config_name, config_value)
                    },
                    StartNodeError::ValueNotGreaterThanZero { node_name, node_id, config_name, config_value, .. } => {
                        format!("开始节点 [{}({})] 配置 [{}] 应该大于零(> 0)，但值为 [{}]", node_name, node_id, config_name, config_value)
                    },
                }
            },
        }
