...
impl crate::error::error_trait::StarRiverErrorTrait for DataProcessorError {
    ...
    fn get_error_message(&self, language: Language) -> String {
        match language {
            ...
            Language::Chinese => {
                match self {
                    DataProcessorError::JsonParsing { .. } => {
                        "JSON解析失败".to_string()
                    },
                    ...
                    DataProcessorError::InvalidFieldType { field, expected, actual, context, .. } => {
                        if let Some(ctx) = context {
                            format!("字段 '{}' 数据类型无效: 期望 {}，实际 {} (上下文: {})", field, expected, actual, ctx)
                        } else {
                            format!("字段 '{}' 数据类型无效: 期望 {}，实际 {}", field, expected, actual)
                        }
                    },
                    DataProcessorError::KlineDataParsing { message, symbol, interval, .. } => {
                    ...
                    DataProcessorError::DataValidation { message, context, field, value, .. } => {
                        if let Some(ctx) = context {
                    ...
                    DataProcessorError::EnumParsing { field, variant, valid_variants, .. } => {
                        format!("枚举解析失败，字段 '{}': 未知变体 '{}', 有效变体: [{}]", 
                               field, variant, valid_variants.join(", "))
                    },
                    DataProcessorError::StreamDataFormat { message, expected_format, actual_data, .. } => {
