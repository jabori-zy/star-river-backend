...
impl VariableNodeContext {
    /// Create async task handle to get custom variable
    pub(super) async fn create_get_custom_var_handle(
        ...
        is_leaf_node: bool,
    ) -> tokio::task::JoinHandle<Result<(), VariableNodeError>> {
        tokio::spawn(async move {
            ...
            match response {
                StrategyResponse::Success { payload, .. } => {
                    ...
                    .into();
                    strategy_output_handle.send(var_event.clone().into())?;
                    if is_leaf_node {
                        let payload = ExecuteOverPayload::new(Some(config_id), Some("handle get custom variable".to_string()));
                        ...
                        output_handle.send(var_event.clone().into())?;
                    }
                }
