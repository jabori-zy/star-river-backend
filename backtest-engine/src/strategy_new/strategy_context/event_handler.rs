        ...
        match command {
            ...
            BacktestStrategyCommand::GetCustomVariableValue(cmd) => {
                ...
                if let Ok(value) = result {
                    ...
                    let resp = GetCustomVariableValueResponse::fail(Arc::new(err));
                    cmd.respond(resp);
                }
            }
            ...
            BacktestStrategyCommand::UpdateCustomVariableValue(cmd) => {
                ...
                if let Ok(value) = result {
                    ...
                    let err = result.unwrap_err();
                    let resp = UpdateCustomVariableValueResponse::fail(Arc::new(err));
                    cmd.respond(resp);
                }
            }
            ...
            BacktestStrategyCommand::ResetCustomVariableValue(cmd) => {
                ...
                if let Ok(value) = result {
                    ...
                    let err = result.unwrap_err();
                    let resp = ResetCustomVariableValueResponse::fail(Arc::new(err));
                    cmd.respond(resp);
                }
            }
            ...
            BacktestStrategyCommand::UpdateSysVariableValue(cmd) => {
                ...
                let payload = UpdateSysVariableRespPayload;
                let resp = UpdateSysVariableValueResponse::success(payload);
                cmd.respond(resp);
            }
