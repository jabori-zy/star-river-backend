...
impl StrategyLifecycle for BacktestStrategy {
    ...
    async fn stop_strategy(&mut self) -> Result<(), Self::Error> {
        // Get current state
        // If strategy current state is Stopped, do not proceed
        ...
        // After sending signals, iterate through all nodes, get node status, if all node states are stopped, update strategy status to Stopped
    ...
    async fn update_strategy_state(&mut self, trigger: Self::Trigger) -> Result<(), Self::Error> {
        // Get all required data in advance to avoid holding references in the loop
        ...
        for action in transition_result.get_actions() {
            ...
            match action {
                ...
                BacktestStrategyStateAction::InitNode => {
                    ...
                    // Pass context reference, let init_node method control lock lifetime internally
                ...
                BacktestStrategyStateAction::StopNode => {
                    ...
                    // Pass context reference, let stop_node method control lock lifetime internally
