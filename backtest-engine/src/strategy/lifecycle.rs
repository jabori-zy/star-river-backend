...
use super::strategy_state_machine::{StrategyRunState, StrategyStateTransTrigger};
use crate::error::strategy_error::{BacktestStrategyError, WaitAllNodesStoppedTimeoutSnafu};
...
impl BacktestStrategy {
    pub async fn check_strategy(&mut self) -> Result<(), BacktestStrategyError> {
        ...
        let update_result = self.update_strategy_state(StrategyStateTransTrigger::Check).await;
        ...
        let update_result = self
            .update_strategy_state(StrategyStateTransTrigger::CheckComplete)
            .await;
    ...
    pub async fn init_strategy(&mut self) -> Result<(), BacktestStrategyError> {
        ...
        let update_result = self.update_strategy_state(StrategyStateTransTrigger::Initialize).await;
        if let Err(e) = update_result {
            self.with_ctx_write_async(|ctx| {
                Box::pin(async move {
                    ctx.update_strategy_status(StrategyRunState::Failed.to_string().to_lowercase()).await
        ...
        let update_result = self
            .update_strategy_state(StrategyStateTransTrigger::InitializeComplete)
            .await;
        if let Err(e) = update_result {
            self.with_ctx_write_async(|ctx| {
                Box::pin(async move {
    ...
    pub async fn stop_strategy(&mut self) -> Result<(), BacktestStrategyError> {
        ...
        self.context
            ...
            .await?;
        let update_result = self.update_strategy_state(StrategyStateTransTrigger::Stop).await;
        if let Err(e) = update_result {
            self.with_ctx_write_async(|ctx| {
                Box::pin(async move {
                    ctx.update_strategy_status(StrategyRunState::Failed.to_string().to_lowercase()).await
        ...
        if all_stopped {
            ...
            let update_result = self.update_strategy_state(StrategyStateTransTrigger::StopComplete).await;
            if let Err(e) = update_result {
                self.with_ctx_write_async(|ctx| {
                    Box::pin(async move {
                        ctx.update_strategy_status(StrategyRunState::Failed.to_string().to_lowercase()).await
