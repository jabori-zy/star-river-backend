...
use strategy_core::{
    NodeType, benchmark::node_benchmark::CompletedCycle, error::node_error::{NodeError, StrategyCommandRespRecvFailedSnafu, StrategyCommandSendFailedSnafu}, event::{log_event::NodeStateLogEvent, node_common_event::CommonEvent}, node::{
        context_trait::{NodeHandleExt, NodeIdentityExt, NodeStateMachineExt}, node_handles::NodeOutputHandle, node_state_machine::StateAction, node_trait::{NodeContextAccessor, NodeLifecycle}
    }
};
...
impl NodeUtils {
    ...
    pub async fn send_info_status_event(
        ...
        let log_event: CommonEvent = NodeStateLogEvent::info(strategy_id, node_id, node_name, node_type.to_string(), state.to_string(), action.to_string(), msg).into();
    ...
    pub async fn send_error_status_event(
        ...
        let log_event: CommonEvent =
            NodeStateLogEvent::error(strategy_id, node_id, node_name, node_type.to_string(), NodeRunState::Failed.to_string(), action.to_string(), error).into();
        let _ = strategy_output_handle.send(log_event.into());
    }
