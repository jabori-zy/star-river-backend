...
impl NodeFunction {
    pub async fn listen_external_event(context: Arc<RwLock<Box<dyn Context>>>){
    ...
    pub async fn listen_message(state: Arc<RwLock<Box<dyn Context>>>) {
    ...
    /// 通用的任务取消实现
    pub async fn cancel_task(state: Arc<RwLock<Box<dyn Context>>>) 
    {
