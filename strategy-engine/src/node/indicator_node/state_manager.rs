...
pub struct IndicatorNodeStateChangeActions {
    ...
    pub actions: Vec<IndicatorNodeStateAction>,
}
...
impl IndicatorNodeStateManager {
    ...
    pub fn transition(&mut self, event: NodeStateTransitionEvent) -> Result<IndicatorNodeStateChangeActions, String> {
        ...
        match (self.current_state.clone(), event) {
            ...
            (NodeRunState::Created, NodeStateTransitionEvent::Initialize) => {
                ...
                Ok(IndicatorNodeStateChangeActions {
                    new_state: NodeRunState::Initializing,
                    actions: vec![IndicatorNodeStateAction::LogTransition, IndicatorNodeStateAction::ListenAndHandleExternalEvents, IndicatorNodeStateAction::ListenAndHandleMessage],
                })
            }
            // 初始化完成，进入Ready状态
            (NodeRunState::Initializing, NodeStateTransitionEvent::InitializeComplete) => {
                ...
                Ok(IndicatorNodeStateChangeActions {
                    ...
                    actions: vec![IndicatorNodeStateAction::LogTransition, IndicatorNodeStateAction::LogNodeState],
                })
            }
            // 从Ready状态开始启动
            (NodeRunState::Ready, NodeStateTransitionEvent::Start) => {
                ...
                Ok(IndicatorNodeStateChangeActions {
                    ...
                    actions: vec![IndicatorNodeStateAction::LogTransition],
                })
            }
            // 启动完成，进入Running状态
            ...
            (_, NodeStateTransitionEvent::Fail(error)) => {
                ...
                Ok(IndicatorNodeStateChangeActions {
                    ...
                    actions: vec![IndicatorNodeStateAction::LogTransition, IndicatorNodeStateAction::LogError(error)],
                })
            }
            // 处理无效的状态转换
            (state, event) => {
