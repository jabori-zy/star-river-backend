...
#[derive(Debug)]
pub struct LiveDataNodeStateChangeActions {
    pub new_state: NodeRunState,
    pub actions: Vec<LiveDataNodeStateAction>,
}


// 状态管理器
#[derive(Debug, Clone)]
pub struct LiveDataNodeStateManager {
    current_state: NodeRunState,
    node_id: String,
    node_name: String,
}

impl LiveDataNodeStateManager {
    ...
    pub fn transition(&mut self, event: NodeStateTransitionEvent) -> Result<LiveDataNodeStateChangeActions, String> {
        ...
        match (self.current_state.clone(), event) {
            ...
            (NodeRunState::Created, NodeStateTransitionEvent::Initialize) => {
                ...
                Ok(LiveDataNodeStateChangeActions {
                    ...
                    actions: vec![LiveDataNodeStateAction::LogTransition, LiveDataNodeStateAction::ListenAndHandleExternalEvents],
                })
            }
            // 初始化完成，进入Ready状态
            (NodeRunState::Initializing, NodeStateTransitionEvent::InitializeComplete) => {
                ...
                Ok(LiveDataNodeStateChangeActions {
                    ...
                    actions: vec![LiveDataNodeStateAction::LogTransition, LiveDataNodeStateAction::LogNodeState],
                })
            }
            // 从Ready状态开始启动
            ...
            (NodeRunState::Running, NodeStateTransitionEvent::Stop) => {
                ...
                Ok(LiveDataNodeStateChangeActions {
                    ...
                    actions: vec![LiveDataNodeStateAction::LogTransition, LiveDataNodeStateAction::UnsubscribeKline],
                })
            }
            // 停止完成，进入Stopped状态
            ...
            (_, NodeStateTransitionEvent::Fail(error)) => {
                ...
                Ok(LiveDataNodeStateChangeActions {
                    ...
                    actions: vec![LiveDataNodeStateAction::LogTransition, LiveDataNodeStateAction::LogError(error)],
                })
            }
            // 处理无效的状态转换
            (state, event) => {
