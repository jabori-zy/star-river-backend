...
impl PositionMutation {
    pub async fn insert_position(
        ...
        match exchange_position.get_exchange() {
            Exchange::Metatrader5(_) => {
                ...
                Ok(position_model.into()) // Convert database model to system model
            }
            _ => {
                return Err(DbErr::Custom("Unsupported exchange".to_string()));
            }
        }
    }
    ...
    pub async fn update_position(
        db: &DbConn,
        latest_position: Position, // Latest position
    ) -> Result<Position, DbErr> {
