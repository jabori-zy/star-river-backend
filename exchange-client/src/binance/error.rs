...
impl StarRiverErrorTrait for BinanceError {
    ...
    fn error_code(&self) -> ErrorCode {
        ...
        let code = match self {
            BinanceError::StateMachineError { .. } => 1001,         // State machine error
            BinanceError::DataProcessorError { .. } => 1002,        // Data processor error
            BinanceError::HttpClientNotCreated { .. } => 1003,      // HTTP client not created
            BinanceError::WebSocketConnectionFailed { .. } => 1004, // WebSocket connection failed
            BinanceError::PingFailed { .. } => 1005,                // Ping failed
            BinanceError::Network { .. } => 1006,                   // Network error
            BinanceError::Response { .. } => 1007,                  // Response error
            BinanceError::ParseServerTimeFailed { .. } => 1008,     // Parse server time failed
            BinanceError::ParseRawDataFailed { .. } => 1009,        // Parse raw data failed
            BinanceError::DateTimeParseFailed { .. } => 1010,       // Parse timestamp failed
            BinanceError::TypeConversionFailed { .. } => 1011,      // Type conversion failed
            BinanceError::ParseNumberFailed { .. } => 1012,         // Parse number failed
            BinanceError::MissingField { .. } => 1013,              // Missing field
            BinanceError::InvalidFieldType { .. } => 1014,          // Invalid field type
            BinanceError::SymbolNotFound { .. } => 1015,            // Symbol not found
            BinanceError::UnsupportedKlineInterval { .. } => 1016,  // Unsupported kline interval
        };
