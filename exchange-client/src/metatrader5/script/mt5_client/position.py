...
class PositionManager:
    ...
    async def close_position(self, ticket: int):
        ...
        if position['type'] == "buy":
            order_type = self.client.client.ORDER_TYPE_SELL
            price = symbol_info_tick['bid']
        else:
            order_type = self.client.client.ORDER_TYPE_BUY
    ...
    async def get_position_by_id(self, position_id: int) -> tuple[bool, list]:
        positions = self.client.client.positions_get(ticket=position_id)
        # 如果是None, 则有错误
    ...
    async def get_positions_by_symbol(self, symbol: str) -> list:
        ...
        positions = self.client.client.positions_get(symbol=symbol)
        # 如果是None, 则有错误
        if positions is None:
            return False, self.client.client.last_error()
