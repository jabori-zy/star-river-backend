...
class MarketManager:
    ...
    async def get_symbols(self) -> list: 
        symbols = self.client.client.symbols_get()
        if symbols is None:
    ...
    async def get_symbol_info(self, symbol: str) -> dict:
        ...
        symbol_info = self.client.client.symbol_info(symbol)
        if not symbol_info:
            return False, self.client.client.last_error()
    ...
    async def get_symbol_info_tick(self, symbol: str) -> dict:
        ...
        symbol_info_tick = self.client.client.symbol_info_tick(symbol)
    ...
    async def get_kline_series_by_time_range(self, symbol: str, time_frame: int, start_time: str, end_time: str) -> list:
        ...
        def _get_history_kline():
            start = datetime.strptime(start_time,'%Y-%m-%d').replace(tzinfo=self.client.timezone)
            end = datetime.strptime(end_time,'%Y-%m-%d').replace(tzinfo=self.client.timezone)
            
            history_kline = self.client.client.copy_rates_range(symbol, time_frame, start, end)
    ...
    async def get_kline_series(self, symbol: str, interval: str, limit: int) -> list:
        ...
        kline_series = self.client.client.copy_rates_from_pos(symbol, time_frame, 0, limit)
    ...
    async def get_latest_kline(self, symbol: str, interval: str) -> tuple[bool, dict]:
        ...
        latest_kline = self.client.client.copy_rates_from_pos(symbol, time_frame, 0, 1)
