...
async def main():
    ...
    print("客户端的MT5模块ID:", id(mt5_client_1.terminal))
    init_result_1 = await mt5_client_1.initialize_terminal(terminal_path="D:/Program Files/MetaTrader 5-1/terminal64.exe")
    if init_result_1[0]:
        login_result_1 = await mt5_client_1.login(account_id=76898751, password="HhazJ520....", server="Exness-MT5Trial5")
        account_info = await mt5_client_1.get_account_info()
        print(account_info[1]["balance"])
    ...
    if init_result_2[0]:
        login_result_2 = await mt5_client_2.login(account_id=76928111, password="HhazJ520....", server="Exness-MT5Trial5")
        account_info = await mt5_client_2.get_account_info()
        print(account_info[1]["balance"])
