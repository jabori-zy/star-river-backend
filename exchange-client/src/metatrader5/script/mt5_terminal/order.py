...
class OrderManager:
    ...
    async def create_order(self, order_type, order_side, symbol, volume, price, tp, sl):
        ...
        if order_type == "limit":
            # 挂单
            action = self.client.terminal.TRADE_ACTION_PENDING
            # 多头
            if order_side == "long":
                type = self.client.terminal.ORDER_TYPE_BUY_LIMIT
                if tp is not None:
            ...
            # 空头
                ...
                type = self.client.terminal.ORDER_TYPE_SELL_LIMIT
                if tp is not None:
                    tp_price = price - tp * point
                if sl is not None:
                    sl_price = price + sl * point
            ...
            request = {
                ...
                "type_time": self.client.terminal.ORDER_TIME_GTC,
                "type_filling": self.client.terminal.ORDER_FILLING_FOK,
            }
        ...
        # 市价单
            ...
            current_price = self.client.terminal.symbol_info(symbol).ask
            # 多头
            if order_side == "long":
                type = self.client.terminal.ORDER_TYPE_BUY
                if tp is not None:
            ...
            # 空头
                ...
                type = self.client.terminal.ORDER_TYPE_SELL
                if tp is not None:
                    tp_price = current_price - tp * point
                if sl is not None:
                    sl_price = current_price + sl * point
    ...
    async def get_order_by_id(self, order_id: int) -> tuple[bool, list]:
        ...
        orders = self.client.terminal.orders_get(ticket=order_id)
    ...
    async def get_order_by_symbol(self, symbol: str):
        ...
        orders = self.client.terminal.orders_get(symbol=symbol)
        if orders is None:
            return False, self.client.terminal.last_error()
    ...
    async def get_history_order_by_id(self, order_id: int):
        orders = self.client.terminal.history_orders_get(ticket=order_id)
    ...
    async def get_history_order_by_position_id(self, position_id: int):
        ...
        orders = self.client.terminal.history_orders_get(position=position_id)
        if orders is None:
            return False, self.client.terminal.last_error()
    ...
    async def get_history_order_by_symbol(self, symbol: int):
        ...
        orders = self.client.terminal.history_orders_get(from_date, to_date, group=symbol_name)
        if orders is None:
            return False, self.client.terminal.last_error()
