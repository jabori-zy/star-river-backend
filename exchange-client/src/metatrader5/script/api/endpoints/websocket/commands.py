...
async def process_command(websocket: WebSocket, message: Dict[str, Any]):
    ...
    if command == "subscribe":
        await handle_subscribe(websocket, terminal_id, data_type, params, frequency)
    elif command == "unsubscribe":
        await handle_unsubscribe(websocket, terminal_id, data_type, params)
    elif command == "list_subscriptions":
        await handle_list_subscriptions(websocket)
    elif command == "update_frequency":
        await handle_update_frequency(websocket, terminal_id, data_type, params, frequency)
    else:
        await websocket.send_json({
            "status": "error",
...
async def handle_subscribe(websocket: WebSocket, terminal_id: int, data_type: str, params: Dict[str, Any], frequency: int):
    """处理订阅命令"""
    if not terminal_id:
        await websocket.send_json({
            "status": "error",
    ...
    # 生成订阅键
    subscription_key = DataTypeHandler.get_subscription_key(data_type, terminal_id, params)
...
async def handle_unsubscribe(websocket: WebSocket, terminal_id: int, data_type: str, params: Dict[str, Any]):
    """处理取消订阅命令"""
    if not data_type:
        await websocket.send_json({
            "status": "error",
...
async def handle_list_subscriptions(websocket: WebSocket):
    ...
    for key, value in subscribed_data.items():
        if websocket in value["websockets"]:
            ...
            elif data_type == "order" and len(key) >= 2:
                subscription_info["params"] = {
                    "account_id": key[1]
                }
            elif data_type == "position" and len(key) >= 2:
                subscription_info["params"] = {
                    "account_id": key[1]
                }
                if len(key) >= 3:
                    subscription_info["params"]["symbol"] = key[2]
            elif data_type == "account" and len(key) >= 2:
                subscription_info["params"] = {
                    "account_id": key[1]
                }
...
async def handle_update_frequency(websocket: WebSocket, terminal_id: int, data_type: str, params: Dict[str, Any], frequency: int):
    """更新订阅频率"""
    if not data_type:
        await websocket.send_json({
            "status": "error",
    ...
    if updated:
        ...
        print(f"更新订阅频率: {subscription_key} -> {frequency}ms")
    else:
        await websocket.send_json({
            "status": "error",
            "message": f"未找到订阅 {data_type}"
