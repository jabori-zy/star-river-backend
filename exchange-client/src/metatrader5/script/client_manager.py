...
class Mt5ClientManager:
    ...
    def create_terminal(self, terminal_id: int) -> Mt5Client:
        if terminal_id not in self.terminals:
            mt5_module = get_mt5_module(terminal_id)
            self.terminals[terminal_id] = Mt5Client(terminal_id, mt5_module)
        return self.terminals[terminal_id]
    

    
    def get_terminal(self, terminal_id: int) -> Mt5Client:
        if terminal_id not in self.terminals:
            return None
        return self.terminals[terminal_id]
