        ...
        match self {
            ...
            _ => {
                ...
                let code = match self {
                    ...
                    Mt5Error::Network { .. } => 1001,                  // Network error
                    Mt5Error::Server { .. } => 1002,                   // Server error
                    Mt5Error::NoSuccessFieldInResponse { .. } => 1002, // Missing success field in response
                    Mt5Error::HttpClientNotCreated { .. } => 1003,     // HTTP client not created
                    Mt5Error::Json { .. } => 1004,                     // JSON parse error
                    Mt5Error::Response { .. } => 1005,                 // Response error

                    // Terminal operations (1005-1008)
                    Mt5Error::InitializeTerminalFailed { .. } => 1005, // Initialize terminal failed
                    Mt5Error::GetTerminalInfo { .. } => 1006,          // Get terminal info
                    Mt5Error::GetSymbolList { .. } => 1007,            // Get symbol list
                    Mt5Error::GetSymbolInfo { .. } => 1008,            // Get symbol info
                    Mt5Error::Ping { .. } => 1009,                     // Ping

                    // Market data operations (1010)
                    Mt5Error::GetKlineData { .. } => 1010, // Get kline data

                    // Trading operations (1010-1014)
                    Mt5Error::CreateOrder { .. } => 1011,         // Create order
                    Mt5Error::GetOrder { .. } => 1012,            // Get order
                    Mt5Error::GetPosition { .. } => 1013,         // Get position
                    Mt5Error::GetDealByPositionId { .. } => 1014, // Get deal by position ID
                    Mt5Error::GetDeal { .. } => 1015,             // Get deal
                    Mt5Error::GetDealByDealId { .. } => 1016,     // Get deal by deal ID
                    Mt5Error::GetDealByOrderId { .. } => 1017,    // Get deal by order ID
                    Mt5Error::GetPositionNumber { .. } => 1018,   // Get position number

                    // Account operations (1018)
                    Mt5Error::GetAccountInfo { .. } => 1019, // Get account info
                    Mt5Error::Retcode { .. } => 1020,        // Get return code

                    // Order operations (1020)
                    Mt5Error::OrderId { .. } => 1021, // Get order ID

                    // WebSocket errors (1016)
                    Mt5Error::WebSocket { .. } => 1022, // WebSocket error

                    // Connection and initialization errors (1017-1019)
                    Mt5Error::Connection { .. } => 1023,     // Connection error
                    Mt5Error::Initialization { .. } => 1024, // Initialization error
