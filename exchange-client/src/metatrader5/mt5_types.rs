...
// Convert KlineInterval to Mt5KlineInterval
...
// Convert Mt5KlineInterval to KlineInterval
    ...
    fn from(value: Mt5OrderType) -> Self {
        match value {
            ...
            Mt5OrderType::OrderTypeCloseBy => {
                tracing::warn!("Encountered unsupported order type OrderTypeCloseBy, will convert to default market order type");
                OrderType::Market // Set default conversion to market order
            }
        }
    }
}
...
impl OriginalPosition for Mt5Position {
    ...
    fn get_create_time(&self) -> DateTimeUtc {
        Utc.timestamp_millis_opt(self.time_msc).single().expect("Failed to convert timestamp to datetime")
    }
