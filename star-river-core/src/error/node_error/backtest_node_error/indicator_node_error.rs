...
impl crate::error::error_trait::StarRiverErrorTrait for IndicatorNodeError {
    ...
    fn error_code(&self) -> ErrorCode {
        ...
        let code = match self {
            IndicatorNodeError::ValueNotGreaterThanOrEqualToZero { .. } => 1001, // 指标节点回测配置值不能小于零
            IndicatorNodeError::ValueNotGreaterThanZero { .. } => 1002, // 指标节点回测配置值不能大于零
            IndicatorNodeError::IndicatorError { .. } => 1003,       // 指标错误
            IndicatorNodeError::DataSourceParseFailed { .. } => 1004, // 数据源解析失败
            IndicatorNodeError::GetKlineDataFailed { .. } => 1005,   // 获取K线数据失败
            IndicatorNodeError::CalculateIndicatorFailed { .. } => 1006, // 计算指标失败
        };
