use crate::{
    EventCenter, EventPublisher, CommandPublisher, EventReceiver, EventCenterError,
    Channel, Event, CommandReceiver, CommandSender
};
...
impl EventCenterSingleton {
    ...
    pub async fn init() -> Result<(), EventCenterError> {
        ...
        EVENT_CENTER_INSTANCE.set(event_center)
    ...
    pub async fn publisher() -> Result<EventPublisher, EventCenterError> {
        let instance = EVENT_CENTER_INSTANCE.get()
    ...
    pub async fn publish(event: Event) -> Result<(), EventCenterError> {
        ...
        // 直接访问 HashMap，无需额外锁
        let sender = center.broadcast_channels.get(&channel)
            .ok_or_else(|| ChannelNotFoundSnafu{
    ...
    pub async fn send_command(command: Command) -> Result<(), EventCenterError> {
        ...
        let (sender, _receiver) = center.command_channels.get(&engine_name)
            .ok_or_else(|| EngineCommandSenderNotFoundSnafu {
    ...
    /// 获取指定引擎的命令接收器
    pub async fn get_command_receiver(engine_name: &EngineName) -> Result<Arc<Mutex<CommandReceiver>>, EventCenterError> {
        let instance = EVENT_CENTER_INSTANCE.get()
            .ok_or_else(|| EventCenterInstanceNotInitializedSnafu{}.build())?;
    ...
    /// 获取指定引擎的命令发送器
    pub async fn get_command_sender(engine_name: EngineName) -> Result<CommandSender, EventCenterError> {
        let instance = EVENT_CENTER_INSTANCE.get()
            .ok_or_else(|| EventCenterInstanceNotInitializedSnafu{}.build())?;
