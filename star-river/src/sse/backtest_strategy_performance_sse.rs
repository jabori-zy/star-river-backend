...
pub async fn backtest_strategy_performance_sse_handler() -> Sse<impl Stream<Item = Result<Event, Infallible>>> {
    ...
    impl Drop for Guard {
        fn drop(&mut self) {
            tracing::info!("{} Backtest Strategy Performance SSE connection disconnected", self.channel_name);
        }
    }
        ...
        while let Some(result) = stream.next().await {
            ...
            let event = match result {
                Ok(EventCenterEvent::Strategy(StrategyEvent::BacktestStrategy(_))) => {
                    ...
                    match event {
                        EventCenterEvent::Strategy(StrategyEvent::BacktestStrategy(BacktestStrategyEvent::StrategyPerformanceUpdate(_))) => {
