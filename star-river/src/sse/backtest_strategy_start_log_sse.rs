...
#[utoipa::path(
    get,
    path = "/api/v1/strategy/backtest/start-log",
...
pub async fn backtest_strategy_start_log_sse_handler(State(star_river): State<StarRiver>,) -> Sse<impl Stream<Item = Result<Event, Infallible>>> {
        ...
        while let Some(result) = stream.next().await {
            ...
            let event = match result {
                Ok(EventCenterEvent::Strategy(StrategyEvent::BacktestStrategy(_))) => {
                    ...
                    match event {
                        EventCenterEvent::Strategy(StrategyEvent::BacktestStrategy(BacktestStrategyEvent::NodeStartLog(_))) |
                        EventCenterEvent::Strategy(StrategyEvent::BacktestStrategy(BacktestStrategyEvent::StrategyStartLog(_))) => {
