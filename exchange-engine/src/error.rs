        ...
        match self {
            ...
            _ => {
                ...
                let code = match self {
                    ...
                    ExchangeEngineError::BinanceError { .. } => 1002,            // 币安错误
                    ExchangeEngineError::Mt5Error { .. } => 1003,                // MetaTrader5错误
                    ExchangeEngineError::EngineStateMachineError { .. } => 1004, // 引擎状态机错误
                    ExchangeEngineError::DatabaseError { .. } => 1005,           // 数据库错误
                    ExchangeEngineError::UnsupportedExchangeType { .. } => 1006, // 不支持的交易所类型

                    // Exchange Client Management (1011-1013)
                    ExchangeEngineError::ExchangeClientNotRegistered { .. } => 1007, // 交易所客户端未找到
                    ExchangeEngineError::BinanceRegisterFailed { .. } => 1008,       // 币安注册失败
                };
