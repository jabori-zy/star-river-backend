...
pub async fn get_exchange_status(
    ...
    let exchange_status = engine_guard.with_ctx_read_async(|ctx| {
        Box::pin(async move {
            ctx.exchange_status(&account_id).await
        })
    })
    .await;
...
pub async fn connect_exchange(
    ...
    let engine = exchange_manager.exchange_engine().await;
    let engine_guard = engine.lock().await;
    let result = engine_guard.with_ctx_write_async(|ctx| {
        Box::pin(async move {
            ctx.register_exchange(account_id).await
        })
    })
    .await;
    match result {
