...
pub async fn get_strategy_by_id(
    ...
    // Simulate loading state, sleep 3 seconds
...
#[utoipa::path(
    ...
    params(
        ("strategy_id" = i32, Path, description = "The ID of the strategy to delete")
    ),
    responses(
        (status = 200, description = "Strategy deleted successfully", content_type = "application/json" ),
        (status = 400, description = "Strategy deletion failed", content_type = "application/json")
    )
)]
