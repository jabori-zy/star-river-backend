...
#[utoipa::path(
    ...
    tag = BACKTEST_CONTROL_TAG,
    summary = "Get play index",
    params(
        ("strategy_id" = i32, Path, description = "The ID of the strategy to get play index")
    ),
    responses(
        (status = 200, description = "Get play index successfully"),
        (status = 400, description = "Get play index failed")
    )
)]
...
#[utoipa::path(
    ...
    summary = "Reset playback",
    params(
        ("strategy_id" = i32, Path, description = "The ID of the strategy to reset")
    ),
    responses(
        (status = 200, description = "Reset strategy successfully"),
        (status = 400, description = "Reset strategy failed")
    )
)]
