...
use engine_context::StrategyEngineContext;
...
impl BacktestEngine {
    ...
    async fn get_strategy_context(&self, strategy_id: i32) -> Result<Arc<RwLock<BacktestStrategyContext>>, StrategyEngineError> {
        let context = self.context.read().await;
        let strategy_context: &StrategyEngineContext = context.as_any().downcast_ref::<StrategyEngineContext>().unwrap();
