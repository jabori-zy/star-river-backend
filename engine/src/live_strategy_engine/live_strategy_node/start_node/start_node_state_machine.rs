...
impl LiveNodeTransitionAction for StartNodeStateAction {
...
impl LiveStateChangeActions for StartNodeStateChangeActions {
...
impl LiveNodeStateMachine for StartNodeStateMachine {
    fn as_any(&self) -> &dyn Any {
        self
    }
    ...
    fn transition(
        ...
        match (self.current_state.clone(), event) {
            ...
            (state, event) => {
                ...
                Err(format!(
                    "节点 {}({}): 无效的状态转换: {:?} -> {:?}",
                    self.node_name, self.node_id, state, event
                ))
            }
        }
    }
