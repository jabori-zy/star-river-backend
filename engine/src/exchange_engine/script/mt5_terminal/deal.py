...
class DealManager:
    ...
    async def get_deals_by_position_id(self, position_id: int) -> tuple[bool, list]:
        ...
        deals = self.client.terminal.history_deals_get(position=position_id)
        if deals is None:
    ...
    async def get_deals_by_deal_id(self, deal_id: int):
        ...
        deals = self.client.terminal.history_deals_get(ticket=deal_id)
        if deals is None:
            return False, self.client.terminal.last_error()
    ...
    async def get_deals_by_symbol(self, symbol: str):
        ...
        deals = self.client.terminal.history_deals_get(from_date, to_date, group=symbol_name)
        if deals is None:
            return False, self.client.terminal.last_error()
