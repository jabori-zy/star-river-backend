...
class Mt5Terminal:
    def __init__(self, client_id: int, terminal) -> None:
    ...
    async def login(self, account_id, password, server) -> tuple[bool, dict]:
        terminal_info = await self.get_terminal_info()
        if not terminal_info[0]:
            return False, self.terminal.last_error()
        
        login_result = self.terminal.login(account_id, password, server)
