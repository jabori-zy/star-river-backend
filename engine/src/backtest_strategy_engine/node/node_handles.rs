...
impl NodeOutputHandle {
    ...
    pub fn subscribe(&mut self, subscriber_id: String) -> broadcast::Receiver<BacktestNodeEvent> {
        ...
        tracing::debug!("{} receiver_count: {}", self.output_handle_id,self.receiver_count());
        receiver
    }
...
impl std::fmt::Display for NodeOutputHandle {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "NodeOutputHandle(node_id: {}, output_handle_id: {}, receiver_count: {}, subscriber: {:?})", self.node_id, self.output_handle_id, self.receiver_count(), self.subscriber)
    }
}
