...
impl StrategyEngineContext {
    pub async fn init(&mut self, strategy_id: i32) -> Result<(), StrategyEngineError> {
        // 检查是否已经在初始化或已存在
    ...
    // 停止回测策略
    pub async fn stop(&mut self, strategy_id: i32) -> Result<(), StrategyEngineError> {
    ...
    pub async fn get_backtest_strategy_keys(&self, strategy_id: i32) -> HashMap<Key, NodeId> {
        ...
        if let Ok(strategy) = strategy {
            strategy.get_context().read().await.keys().await
        } else {
            HashMap::new()
        }
    }

    pub async fn get_strategy_status(&self, strategy_id: i32) -> Result<String, StrategyEngineError> {
        // 检查是否正在初始化或有策略实例
