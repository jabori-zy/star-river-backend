...
use super::{CycleTracker, KlineNodeContext};
use crate::backtest_engine::node::node_handles::NodeOutputHandle;
use crate::backtest_engine::node::node_context_trait::{NodeBenchmark, NodeCommunication, NodeEventEmitter, NodeEventHandler, NodeHandleTrait, NodeIdentity, NodePlayback, NodeRelation};
use crate::backtest_engine::node::node_list::kline_node::state_machine::KlineNodeAction;
use event_center::communication::Response;
use event_center::communication::backtest_strategy::{
    BacktestNodeCommand, GetKlineDataCmdPayload, GetKlineDataCommand, GetMinIntervalSymbolsCommand, NodeResetResponse, UpdateKlineDataCmdPayload, UpdateKlineDataCommand
};
use event_center::event::node_event::backtest_node_event::{BacktestNodeEvent, StartNodeEvent};
...
use star_river_core::error::node_error::backtest_node_error::kline_node_error::KlineNodeError;
use star_river_core::error::node_error::backtest_node_error::kline_node_error::{
...
impl KlineNodeContext {
    ...
    async fn insert_new_kline_to_strategy(
        ...
        let update_cmd = UpdateKlineDataCommand::new(self.node_id().clone(), resp_tx, Some(update_paylod));
    ...
    async fn update_existing_kline(
        ...
        let get_last_kline_cmd = GetKlineDataCommand::new(self.node_id().clone(), resp_tx, Some(payload));
        ...
        if response.is_success() {
            ...
            let update_cache_params = UpdateKlineDataCommand::new(self.node_id().clone(), resp_tx, Some(payload));
    ...
    async fn handle_min_interval_kline(
        ...
        if *pre_kline_timestamp != kline_timestamp && *pre_kline_timestamp == 0 {
            *pre_kline_timestamp = kline_timestamp;
            ...
            .into();
            self.strategy_output_handle().send(time_update_event.into()).unwrap();
        }
    ...
    pub async fn get_min_interval_symbols(&mut self) -> Result<Vec<KlineKey>, String> {
        ...
        let cmd = GetMinIntervalSymbolsCommand::new(self.node_id().clone(), tx, None);
...
#[async_trait]
impl NodeEventHandler<KlineNodeAction> for KlineNodeContext {
