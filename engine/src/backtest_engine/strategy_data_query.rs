...
use super::engine_context::StrategyEngineContext;
...
impl BacktestEngine {
    ...
    /// 获取策略性能报告
    pub async fn get_strategy_performance_report(&self, strategy_id: StrategyId) -> Result<StrategyPerformanceReport, StrategyEngineError> {
    ...
    /// 获取虚拟订单列表
    pub async fn get_virtual_orders(&self, strategy_id: StrategyId) -> Result<Vec<VirtualOrder>, StrategyEngineError> {
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
    ...
    /// 获取当前持仓
    pub async fn get_current_virtual_positions(&self, strategy_id: StrategyId) -> Result<Vec<VirtualPosition>, StrategyEngineError> {
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
    ...
    /// 获取历史持仓
    pub async fn get_history_virtual_positions(&self, strategy_id: StrategyId) -> Result<Vec<VirtualPosition>, StrategyEngineError> {
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
    ...
    /// 获取交易明细
    pub async fn get_virtual_transactions(&self, strategy_id: StrategyId) -> Result<Vec<VirtualTransaction>, StrategyEngineError> {
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
    ...
    /// 获取运行日志
    pub async fn get_running_log(&self, strategy_id: StrategyId) -> Result<Vec<StrategyRunningLogEvent>, StrategyEngineError> {
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
        Ok(ctx_guard.running_log().await)
    }
    ...
    pub async fn get_strategy_data_by_datetime(
        ...
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
        Ok(ctx_guard.get_strategy_data_by_datetime(key, datetime, limit).await?)
    }

    /// 获取策略变量
    pub async fn get_strategy_variable(&self, strategy_id: StrategyId) -> Result<Vec<StrategyVariable>, StrategyEngineError> {
        let ctx = self.get_strategy_context(strategy_id).await?;
        let ctx_guard = ctx.read().await;
