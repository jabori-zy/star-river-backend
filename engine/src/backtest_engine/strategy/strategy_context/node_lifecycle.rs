...
use crate::backtest_engine::node::NodeRunState;
    ...
    pub async fn init_node(context: Arc<RwLock<Self>>) -> Result<(), BacktestStrategyError> {
        ...
        for n in nodes {
            ...
            match tokio::time::timeout(Duration::from_secs(120), node_handle).await {
                ...
                Err(e) => {
                    return Err(NodeInitTimeoutSnafu { node_name }.into_error(e));
                }
            }
            ...
            if retry_count >= max_retries {
                return Err(NodeStateNotReadySnafu { node_name: node_name }.fail()?);
            }
        }
