...
impl BacktestStrategyFunction {
    pub async fn add_edge(
        ...
        let from_handle_id = edge_config.get("sourceHandle")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EdgeConfigMissFieldSnafu {
                strategy_id: context_guard.strategy_id,
                strategy_name: context_guard.strategy_name.clone(),
                field_name: "sourceHandle".to_string(),
            }.build())?;

        let from_node_id = edge_config.get("source")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EdgeConfigMissFieldSnafu {
                strategy_id: context_guard.strategy_id,
                strategy_name: context_guard.strategy_name.clone(),
        ...
        let to_node_id = edge_config.get("target")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EdgeConfigMissFieldSnafu {
                strategy_id: context_guard.strategy_id,
                strategy_name: context_guard.strategy_name.clone(),
        ...
        let to_handle_id = edge_config.get("targetHandle")
            .and_then(|v| v.as_str())
            .ok_or_else(|| EdgeConfigMissFieldSnafu {
                strategy_id: context_guard.strategy_id,
                strategy_name: context_guard.strategy_name.clone(),
        ...
        let (source, target) = {
            let source = context_guard.node_indices.get(from_node_id)
                .copied()
                .ok_or_else(|| NodeNotFoundSnafu {
                    strategy_id: context_guard.strategy_id,
                    strategy_name: context_guard.strategy_name.clone(),
                    node_id: from_node_id.to_string(),
                }.build())?;
        
            let target = context_guard.node_indices.get(to_node_id)
                .copied()
                .ok_or_else(|| NodeNotFoundSnafu {
                    strategy_id: context_guard.strategy_id,
                    strategy_name: context_guard.strategy_name.clone(),
        ...
        let from_node_output_handle = context_guard.graph.node_weight(source).unwrap().get_output_handle(&from_handle_id.to_string()).await;
        
        tracing::debug!("{}: from_node_output_handle: {:?}", from_handle_id, from_node_output_handle);
        // 增加源节点的出口连接数
        context_guard.graph.node_weight_mut(source).unwrap().add_output_handle_connect_count(&from_handle_id.to_string()).await;
