...
// impl Clone for BacktestStrategyContext {
//     fn clone(&self) -> Self {
//         Self {
...
#[async_trait]
impl StrategyContext for BacktestStrategyContext {
    ...
    async fn handle_command(&mut self, command: NodeCommand) -> Result<(), String> {
        ...
        match command {
            NodeCommand::Strategy(strategy_command) => {
                match strategy_command {
                    StrategyCommand::GetStrategyCacheKeys(params) => {
                        // tracing::info!("{}: 收到命令: {:?}", self.get_strategy_name(), params);
                        let cache_keys = self.get_cache_keys().await;
                        let cache_keys_response = GetStrategyCacheKeysResponse {
                            code: 0,
                            message: "success".to_string(),
                            cache_keys: cache_keys,
                            response_timestamp: get_utc8_timestamp_millis(),
                        };
                        let response = StrategyResponse::GetStrategyCacheKeys(cache_keys_response);
