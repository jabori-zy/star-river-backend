    ...
    async fn create_order2(&mut self) {
        ...
        match self.base_context.trade_mode {
            TradeMode::Live => {
                ...
                let is_exchange_registered = {
                    let exchange_engine_guard = self.exchange_engine.lock().await;
                    exchange_engine_guard.is_registered(&self.live_config.as_ref().unwrap().selected_live_account.account_id).await
                };
                if !is_exchange_registered {
                    tracing::error!("{}: 交易所未注册", self.get_node_id());
                    return;
                }
                ...
                let exchange = exchange_engine_context_guard.get_exchange_ref(&self.live_config.as_ref().unwrap().selected_live_account.account_id).await.unwrap();
                let create_order_params = CreateOrderParams {
                    base_params: BaseCommandParams {
                        strategy_id: self.get_strategy_id().clone(),
                        node_id: self.get_node_id().clone(),
                ...
                if let Ok(order) = OrderMutation::insert_order(&self.database, self.get_strategy_id().clone(), self.get_node_id().clone(), self.live_config.as_ref().unwrap().selected_live_account.account_id, original_order.clone()).await {
                    ...
                    if original_order.get_order_status() == OrderStatus::Filled {
                        ...
                        //     response_id: create_order_params.base_params.request_id,
                        //     order: Some(order),
                        // }));
                        // tracing::info!("发送创建订单响应: {:?}", response_event);
                        // self.get_event_publisher().publish(response_event.into()).unwrap(); // 发送创建订单响应
                        // self.get_event_publisher().publish(order_event.into()).unwrap(); // 发送订单已成交事件
                    } 
