...
use event_center::command::market_engine_command::{
    MarketEngineCommand, SubscribeKlineStreamParams, UnsubscribeKlineStreamParams,
};
...
use types::strategy::node_event::{BacktestNodeEvent, KlineSeriesMessage};
...
impl KlineNodeContext {
    ...
    pub async fn register_exchange(&mut self) -> Result<Response, String> {
        ...
        self.get_command_publisher()
            .send(register_exchange_command.into())
            .await
            .unwrap();
    ...
    pub async fn subscribe_kline_stream(&mut self) -> Result<Response, String> {
        ...
        self.get_command_publisher()
            .send(subscribe_kline_stream_command.into())
            .await
            .unwrap();
    ...
    pub async fn unsubscribe_kline_stream(&mut self) -> Result<Response, String> {
        ...
        tracing::debug!(
            ...
            unsubscribe_kline_stream_command
        );
        self.get_command_publisher()
            .send(unsubscribe_kline_stream_command.into())
            .await
            .unwrap();
            ...
            if let Ok(cache_reponse) = CacheEngineResponse::try_from(response) {
                match cache_reponse {
                    CacheEngineResponse::GetCacheData(get_cache_data_response) => {
                        ...
                        output_handle
                            .send(BacktestNodeEvent::KlineSeries(kline_series_message))
                            .unwrap();
                    }
                    _ => {}
                }
