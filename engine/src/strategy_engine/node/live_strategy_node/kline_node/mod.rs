    ...
    async fn start(&mut self) -> Result<(), String> {
        ...
        let is_registered_and_subscribed = {
            ...
            if let Some(kline_node_context) =
                state_guard.as_any().downcast_ref::<KlineNodeContext>()
            {
                let is_subscribed = kline_node_context.stream_is_subscribed.read().await.clone();
                let is_registered = kline_node_context
                    .exchange_is_registered
        ...
        if !is_registered_and_subscribed {
            tracing::warn!(
                "{}: 交易所未注册或K线流未订阅, 不启动",
                context.read().await.get_node_id()
            );
    ...
    async fn stop(&mut self) -> Result<(), String> {
        ...
        loop {
            let is_subscribed = {
                ...
                {
                    let is_subscribed =
                        kline_node_context.stream_is_subscribed.read().await.clone();
                    is_subscribed
                } else {
                    false
                }
                ...
                match kline_node_state_action {
                    ...
                    KlineNodeStateAction::UnsubscribeKline => {
                        ...
                        if should_stop {
                            ...
                            {
                                let response =
                                    kline_node_context.unsubscribe_kline_stream().await?;
                                if response.success() {
