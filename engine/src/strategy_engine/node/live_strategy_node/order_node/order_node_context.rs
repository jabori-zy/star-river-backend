...
impl OrderNodeContext {
    ...
    async fn get_transaction_detail(&mut self, order: Order) -> Result<(), String> {
        ...
        let transaction_detail = exchange
            .get_transaction_detail(get_transaction_detail_params)
            .await
            .unwrap();
        // 入库
    ...
    async fn create_order(&mut self) {
        ...
        let exchange = self
            .get_exchange(&self.live_config.selected_live_account.account_id)
            .await
            .unwrap();
        ...
        let original_order = exchange
            .create_order(create_order_params.clone())
            .await
            .unwrap();
        if let Ok(order) = OrderMutation::insert_order(
            &self.database,
            self.get_strategy_id().clone() as i64,
            self.get_node_id().clone(),
            self.live_config.selected_live_account.account_id,
        ...
        .await
        {
            ...
            if original_order.get_order_status() == OrderStatus::Filled {
                ...
                let output_handle = self
                    ...
                    .get("order_node_output")
                    .unwrap();
                let order_message = OrderEvent::OrderFilled(order.clone());
                // 发送消息
                output_handle
                    .send(BacktestNodeEvent::Order(order_message.clone()))
                    .unwrap();
                // 获取交易明细
            ...
            match exchange {
                Ok(exchange) => {
                    ...
                    if latest_order.order_status == OrderStatus::Filled {
                        // 更新数据库
                        OrderMutation::update_order(&database, latest_order.clone())
                            .await
                            .unwrap();
                        // 删除未成交的订单
