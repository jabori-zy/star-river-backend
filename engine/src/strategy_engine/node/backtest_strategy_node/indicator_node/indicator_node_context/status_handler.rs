use super::{
    BacktestNodeContextTrait, CalculateHistoryIndicatorCmdPayload, CalculateHistoryIndicatorCommand,
    EventCenterSingleton, GetIndicatorLookbackCmdPayload, GetIndicatorLookbackCommand, GetKlineDataCmdPayload,
    GetKlineDataCommand, IndicatorEngineCommand, IndicatorNodeContext, InitIndicatorDataCmdPayload,
    InitIndicatorDataCommand, Response,
};
...
impl IndicatorNodeContext {
    pub(crate) async fn init_indicator_lookback(&mut self) {
        for keys in self.indicator_keys.keys() {
            ...
            let cmd: IndicatorEngineCommand =
                GetIndicatorLookbackCommand::new(self.get_node_id().clone(), resp_tx, Some(payload)).into();
    ...
    pub async fn calculate_indicator(&self) -> Result<bool, String> {
        ...
        for (ind_key, _) in self.indicator_keys.iter() {
            ...
            self.get_strategy_command_sender()
                .send(get_kline_series_cmd.into())
                .await
                .unwrap();
            let response = resp_rx.await.unwrap();
            if response.is_success() {
                ...
                let cmd: IndicatorEngineCommand =
                    CalculateHistoryIndicatorCommand::new(node_id.clone(), resp_tx, Some(payload)).into();
                ...
                if response.is_success() {
                    ...
                    let update_indicator_params =
                        InitIndicatorDataCommand::new(node_id.clone(), resp_tx, Some(payload));
                    self.get_strategy_command_sender()
