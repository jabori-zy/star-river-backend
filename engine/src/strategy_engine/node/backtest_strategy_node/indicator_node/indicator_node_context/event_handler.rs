use super::{
    ...
    EventCenterSingleton, GetMinIntervalSymbolsCommand, Indicator, IndicatorEngineCommand, IndicatorNodeContext,
    KeyTrait, KlineKey, Response, UpdateIndicatorDataCmdPayload, UpdateIndicatorDataCommand,
};
...
impl IndicatorNodeContext {
                ...
                for (indicator_key, (config_id, output_handle_id)) in indicator_keys.iter() {
                    ...
                    if let Ok(indicator_data) = calculate_reuslt {
                        ...
                        let update_result = self
                            .update_strategy_indciator_data(indicator_key, last_indicator.clone())
                            .await;
    ...
    pub async fn request_calculate_indicator(
        ...
        let cmd: IndicatorEngineCommand =
            CalculateHistoryIndicatorCommand::new(self.get_node_id().clone(), resp_tx, Some(payload)).into();
