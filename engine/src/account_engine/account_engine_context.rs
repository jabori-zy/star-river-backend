...
impl AccountEngineContext {
    ...
    async fn update_monitor_accounts(&mut self, account_id: i32) {
        ...
        let new_account = AccountConfigQuery::get_account_config_by_id(&self.database, account_id)
            .await
            .unwrap();
                    ...
                    match exchange {
                        Ok(exchange) => {
                            ...
                            match account_info {
                                Ok(account_info) => {
                                    // 1.更新数据库
                                    let account_info = AccountInfoMutation::update_account_info(
                                        &database,
                                        account.get_account_id(),
                                        account_info.to_json(),
                                    )
                                    .await
                                    .unwrap();
                                    // 2.更新账户信息
